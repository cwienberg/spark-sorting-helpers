...
private[sorting] object SortHelpers {
  ...
  def repartitionAndSort[K: Ordering, V: ClassTag, A: Ordering](
    ...
    rdd
      ...
      .mapPartitions(_.map { case (SecondarySortKey(k, _), v) => (k, v) }, preservesPartitioning = true)
  }
  ...
  def joinAndApply[K, R, V, A](op: R => V => A)(resourcesIter: Iterator[(K, R)], valuesIter: Iterator[(K, Iterator[V])])(implicit keyOrdering: Ordering[K]): Iterator[(K, A)] = {
